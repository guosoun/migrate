...
import user from 'hydrooj/src/service/layers/user';
...
export async function run({
    ...
    */
    ...
    const grp: Map<string, number[]> = new Map();
    const superAdmin = [];
    ...
    for (const udoc of udocs) {
        ...
        if (current) {
            ...
            await UserModel.setById(uid, {
                ...
                beginAt: new Date(udoc.start_time * 1000),
                endAt: new Date(udoc.end_time * 1000),
            });
            ...
            // udoc.group_id is a string like "1|2|3"
            if (udoc.group_id) {
                for (const gid of udoc.group_id.split('|')) {
                    if (!grp.has(gid)) grp.set(gid, []);
                    grp.get(gid).push(uid);
                }
            }
        }
    }

    for (const [gid, uids] of grp.entries()) {
        UserModel.updateGroup(domainId, gid, uids);
    }
    ...
    */
    ...
    for (const tdoc of tdocs) {
        ...
        const tid = await ContestModel.add(
            ...
            pids, ratedTids.includes(tdoc.id), { maintainer: tdoc.admins.split('|').map((i) => uidMap[i]), assign: tdoc.group_id.split('|') },
        );
